...
pub contract MiniKitties: NonFungibleToken {
  ...
  pub resource interface MiniKittiesCollectionPublic {
  ...
  pub resource Collection: MiniKittiesCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic {
    ...
    pub fun deposit(token: @NonFungibleToken.NFT) {
      let token <- token as! @MiniKitties.NFT
  ...
  pub fun mintNFT(
    ...
    MiniKitties.totalSupply = MiniKitties.totalSupply + UInt64(1)
  }

  init() {
    self.totalSupply = 0

    self.CollectionStoragePath = /storage/MiniKittiesCollection
    self.CollectionPublicPath = /public/MiniKittiesCollection
    ...
    self.account.link<&MiniKitties.Collection{NonFungibleToken.CollectionPublic, MiniKitties.MiniKittiesCollectionPublic}>(
